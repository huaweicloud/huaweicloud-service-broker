...
func (b *RDSBroker) Bind(instanceID, bindingID string, details brokerapi.BindDetails) (brokerapi.Binding, error) {
	...
	if err != nil {
		return brokerapi.Binding{}, fmt.Errorf("Unable to retrieve instance: %s", err)
	}
	...
	if len(freshInstances.Instances) != 1 {
		return brokerapi.Binding{}, fmt.Errorf("The rds instance not exist or more than one.")
	}
	...
	credential, err := BuildBindingCredential(instance.DbUserName, instance.Port, "", dbusername, dbpassword, service.Name)
	if err != nil {
