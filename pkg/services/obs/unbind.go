...
func (b *OBSBroker) Unbind(instanceID, bindingID string, details brokerapi.UnbindDetails) error {
	...
	err := database.BackDBConnection.
		...
		Count(&length).Error
	if err != nil {
		return fmt.Errorf("check obs bind length in back database failed. Error: %s", err)
	}
	...
	err = database.BackDBConnection.
		...
		Count(&length).Error
	if err != nil {
		return fmt.Errorf("check obs instance length in back database failed. Error: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("delete obs bind in back database failed. Error: %s", err)
	}
